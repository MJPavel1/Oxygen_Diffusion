...
# ODE system
# ---------------------------
def make_rhs(mass_dirty: float, mass_scav: float, tau: float, Csat_scav: float):
    """Returns the RHS function for solve_ivp, closed over simulation parameters."""
...
# ---------------------------
def run(
    T: float = 1200 + 273.15,   # K
    Cd0: float = 700.0,          # wt ppm oxygen in dirty powder
    Cs0: float = 50.0,           # wt ppm oxygen in scavenger
    mass_dirty: float = 1.0,
    mass_scav: float = 50.0,
    Rd: float = 17e-6,           # m
    Rs: float = 25e-6,           # m
) -> None:
    ...
    # Solve
    rhs = make_rhs(mass_dirty, mass_scav, tau, Csat_scav)
